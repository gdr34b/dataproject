...
Another section. Something something start related work. 

A paper by Richard Franke \cite{franke1982scattered} published in 1982 looks at the interpolation of scattered data. They evaluate different computational methods towards obtaining a smooth function F(x, y) to follow the dataset. They cover seven different methods in their evaluation: inverse distance weighted method, rectangle based blending method, triangle based blending method, finite element based method, Foley's method, global basis function type method, and modified Maude method. All of these methods focus only on developing a function in order to interpolate scattered data sets.

A later paper by Michael Floater and Armin Iske \cite{floater1996multistep} also interpolates scattered data sets, instead using classical radial basis functions, such as Duchon's thin plate splines and Hardy's multiquadratics. Their research focuses on trying to utilize smooth compactly supported radial basis functions, in order to shorten the excessive computation times that result from applying these functions to large data sets. 

Two potential examples, the Australian example \cite{jeffrey2001using}. And the MODIS example \cite{zhao2005improvements}.

Need to find more on data mining for interpolation. If exists.
